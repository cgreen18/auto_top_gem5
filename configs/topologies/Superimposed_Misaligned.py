...
class Superimposed_Misaligned(SimpleTopology):
    description='Superimposed_Misaligned'
    ...
    def makeTopology(self, options, network, IntLink, ExtLink, Router):
        ...
        n_mems = options.num_mems
        n_cpus = options.num_cpus - n_mems
        ...
        cpu_caches = caches[:n_cpus]
        mem_caches = caches[n_cpus:]
        dirs = [n for n in nodes if n.type == 'Directory_Controller']

        print(f'caches({len(caches)})={caches}')
        print(f'cpu_caches({len(cpu_caches)})={cpu_caches}')
        print(f'mem_caches({len(mem_caches)})={mem_caches}')
        print(f'dirs({len(dirs)})={dirs}')

        assert(n_cpus == len(cpu_caches))
        assert(n_mems == len(mem_caches))
        assert(n_dirs == len(dirs))

        # nothing else (dma)
        assert(n_cpus + n_mems + n_dirs == len(nodes))
        ...
        for idx, router in enumerate(routers):
            ...
            for i in range(dirs_per_router):
                src = n_dir_conns
                dest = idx
        ...
        for idx, router in enumerate(routers):
            ...
            for i in range(mems_per_router):
                src = n_mem_conns

                print(f'Adding external link: l1 cache node {src} <-> router {dest} ')
                ext_links.append(ExtLink(link_id=link_count, ext_node=mem_caches[src],
                                        int_node=router,latency=link_latency))
